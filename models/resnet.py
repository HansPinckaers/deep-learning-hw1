...
class RandomZeroPadding(torch.nn.Module):
    def __init__(self, padding):
        super(RandomZeroPadding, self).__init__()
    ...
    def forward(self, input):
        ...
        def pad_rows(arr, prev_mask=None):
            ...
            if prev_mask is not None:
                mask_output = torch.zeros(prev_mask.shape[0], prev_mask.shape[1],
                                          prev_mask.shape[2] + padding * 2, dtype=prev_mask.dtype).cuda()
                mask_output.masked_scatter_(mask[0], prev_mask)
                mask = mask_output
            return output, mask

        input, mask = pad_rows(input)

        # transpose
        input = input.transpose(2, 3)
        mask = mask.transpose(1, 2)
        input, mask = pad_rows(input, mask)
    ...
    def backward(self, grad_output):
        return grad_output.masked_select(self.mask)
